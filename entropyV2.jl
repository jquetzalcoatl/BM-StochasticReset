...
op = [firstPassage(0.1, func="ToggleGaussian", x0=[1,2], σ=[1,2]) for i in 1:10]
...
begin
    ...
    Threads.@threads for δc in 0.4:0.2:3.0
        @info δc
        global fpstats = vcat(fpstats,simPoints([δc, 2.7], 0.1, 50,z0=[0.01, 2], D=1, Δw=0.5, sampsize=5000, func="Gaussian"))
    end
...
begin
    f = plot()
    for (j,δc) in enumerate(1.8:0.2:2.6)
        f = plot!(0:0.01:10, w-> entPerRes(w)*nmean(δc,w), label=δc, frame=:box, lw=5, s=:auto)
        r = fpstats[fpstats[:, end] .== δc,:]
        f = plot!(sqrt.([r[i,1] for i in 1:1:size(r,1)]) .* (0.01/(2 * δc)), [r[i,4] for i in 1:1:size(r,1)], seriestype=:scatter, markershape=:auto, legend=:none, markerstrokewidth=0, ms=12)
    end
    f = vline!([exp(-γ/2)], lw=4, c=:black, ls=:dash)
    f = plot!(size=(1500,900), leftmargin = 10Plots.mm, bottommargin = 10Plots.mm, ylabel="ΔΣ", xlabel="w", guidefontsize=25, tickfont = font(23, "Helvetica"), yscale=:linear, xscale=:linear, xlim=(0,3), ylim=(-8,110))
...
savefig(f, "/Users/javier/Dropbox/Aplicaciones/Overleaf/MFPT/Figs/MFPEC_Gauss.png")
...
tt2GsLim(ceven,weven, codd, wodd; z0=1, D=1, z1=1) = weven>10 ? min(tt2Gs(ceven,weven, codd, wodd, z0, D), abs(exp(ceven^2)*weven*√π-1)*2/(1+weven/wodd*exp(-ceven^2 * ((z1/z0 *weven/wodd)^2-1)))*(z0^2/(4*weven^2*ceven^2*D))) : tt2Gs(ceven,weven, codd, wodd, z0, D)
...
nmeanLim(ceven,weven, codd, wodd; z0=1, D=1, z1=1) = weven>10 ? min(nmean(ceven,weven, codd, wodd), abs(exp(ceven^2)*weven*√π-1)*2/(1+weven/wodd*exp(-ceven^2 * ((z1/z0 *weven/wodd)^2-1)))) : nmean(ceven,weven, codd, wodd)
entPerRes(w) = 0.5*log(w^2 * exp(γ))
...
begin
    ...
    fpstats = zeros(1,6)
    for α in 1:0.5:3
        Threads.@threads for δc in 1.4:0.2:3.0
            @info δc, α
            tmp = simPoints([δc, δc/α], 0.1, 20, z0=[0.1, 0.1], D=1, Δw=0.1, sampsize=5000, func="ToggleGaussian")
...
writedlm(pwd() * "/MFPT/mfpstats2Gauss-B.txt", fpstats)
...
begin
    z0 = 0.1
    f = plot()
    for α in 3.0:0.5:3.0
        for (j,δc) in enumerate(1.8:0.2:3.0)

            # f = plot!(0.01:0.01:20, w->2*tt2Gs(δc,w, δc/α, α*w), label=δc, frame=:box, lw=5, s=:auto)
            f = plot!(0.01:0.01:20, w->2*tt2GsLim(δc,w, δc/α, α*w, z0=1,z1=1), label=δc, frame=:box, lw=5, s=:auto)
...
begin
    ...
    for α in 1.0:0.5:2.0
        for (j,δc) in enumerate(1.4:0.2:2.6)
            f = plot!(0.1:0.01:10, w-> nmeanLim(δc,w, δc/α, α*w, z0=1,z1=1), s=:auto, label="c = $(δc)", frame=:box, lw=5, legend=:topleft)
            r = fpstats[(fpstats[:, 5] .== δc) .* (fpstats[:, 6] .== α), :]
...
begin
    z0 = 0.01
    f = plot()
    for α in 1.0:0.5:1.0
        for (j,δc) in enumerate(1.2:0.2:3.0)
            f = plot!(log(0.1):0.01:log(500), u-> log(nmeanLim(δc,exp(u), δc/α,α*exp(u), z0=1,z1=1)), frame=:box, lw=5, legend=:bottomright, s=:auto, label="c = $(δc), α = $(α)")
            r = fpstats[(fpstats[:, 5] .== δc) .* (fpstats[:, 6] .== α), :]
    ...
    f = vline!([-γ/2], lw=4, c=:black, ls=:dash, label=:none)
    f = plot!(size=(1500,1500), leftmargin = 10Plots.mm, bottommargin = 10Plots.mm, ylabel="ln(μ)", xlabel="ln(w)", guidefontsize=25, tickfont = font(23, "Helvetica"), aspect_ratio=1)
...
begin
    ...
    for α in 1.0:0.5:3.0
        for (j,δc) in enumerate(1.8:0.2:2.6)
            f = plot!(0:0.01:10, w-> totalentropy(δc, w, δc/α, α*w), label=δc, frame=:box, lw=5, s=:auto)
            r = fpstats[(fpstats[:, 5] .== δc) .* (fpstats[:, 6] .== α), :]
            f = plot!(sqrt.([r[i,1] for i in 1:1:size(r,1)]) .* (0.1/(2 * δc)), [r[i,4] for i in 1:1:size(r,1)], seriestype=:scatter, markershape=:auto, legend=:none, markerstrokewidth=0, ms=12)
        end
    end
    f = vline!([exp(-γ/2)], lw=4, c=:black, ls=:dash)
    f = hline!([0], lw=4, c=:black, ls=:dash)
    f = plot!(size=(1500,900), leftmargin = 10Plots.mm, bottommargin = 10Plots.mm, ylabel="ΔΣ", xlabel="w", guidefontsize=25, tickfont = font(23, "Helvetica"), yscale=:linear, xscale=:linear, xlim=(0,2), ylim=(-8,110))
    f = plot(f, ann=[(2.6,15,text("c=1.2", 30,:red, font)),(0.97,100,text("c=3.0", 30,:red, font))])
    # plot(f, ann=(0.97,100,text("c=3.0", 25,:red, font)))
    # plot!( -5:8, (-5:8).^2, inset = (1, bbox(0.0,0.1,0.4,0.4)), subplot = 2)
    # plot!( -5:8, 2*(-5:8).^2, inset = (1, bbox(0.1,0.0,0.4,0.4)), subplot = 2)
...
# function simPoints(c,wmin, wmax; z0=[6],D=1, Δw=0.1, MAXSTEPS=10000000, sampsize=10000, func="Gaussian")
#     σσ = z0 ./ (√2 .* c)
#     # result = [(4*c^2*w^2*D/z0^2, mean([firstPassage(4*c^2*w^2*D/z0^2, x0=z0, σ=σσ, MAXSTEPS=MAXSTEPS, D=D)[idx] for i in 1:sampsize])) for w in wmin:Δw:wmax]
#     mfp = [firstPassage(4*c[1]^2*(wmin)^2*D/z0[1]^2, func=func, x0=z0, σ=σσ, MAXSTEPS=MAXSTEPS, D=D) for i in 1:sampsize]
#     result = hcat(4*c[1]^2*(wmin)^2*D/z0[1]^2, mean( transpose(hcat([collect(mfp[i]) for i in 1:sampsize]...)), dims=1), c[1])
#     # @floop
#     Threads.@threads for w in wmin+Δw:Δw:wmax
#         mfp = [firstPassage(4*c[1]^2*w^2*D/z0[1]^2, func=func, x0=z0, σ=σσ, MAXSTEPS=MAXSTEPS, D=D) for i in 1:sampsize]
#         tmp = hcat(4*c[1]^2*w^2*D/z0[1]^2, mean( transpose(hcat([collect(mfp[i]) for i in 1:sampsize]...)), dims=1), c[1])
#         result = vcat(result, tmp)
#     end
...
begin
    fpstats = simPoints([1.8, 2.7],0.1, 10, z0=[0.1, 2], D=1, Δw=0.1, sampsize=50000, func="Gaussian")
    for δc in 0.4:0.2:3.0
        @info δc
        fpstats = vcat(fpstats,simPoints([δc, 2.7], 0.1, 10,z0=[0.1, 2], D=1, Δw=0.1, sampsize=50000, func="Gaussian"))
    end
end
...
begin
    ...
    for (j,δc) in enumerate(1.8:0.2:2.6)
        ...
        f = plot!(0.01:0.01:50, w->tt2( δc,w, 1), label=δc, frame=:box, lw=5, s=:auto)
        r = fpstats[fpstats[:, end] .== δc,:]
        f = plot!(sqrt.([r[i,1] for i in 1:size(r,1)]) .* (0.1/(2 * δc )), [r[i,2] for i in 1:size(r,1)] ./ 0.1^2, seriestype=:scatter, markershapes = :auto, legend=:none, markerstrokewidth=0, ms=10)
    end
    f = vline!([exp(-γ/2)], lw=4, c=:black, ls=:dash)
    f = plot!(size=(1500,900), leftmargin = 10Plots.mm, bottommargin = 10Plots.mm, ylabel="τD/z₀²", xlabel="w", guidefontsize=25, tickfont = font(23, "Helvetica"), yscale=:log, xscale=:linear)
    f = plot(f, ann=[(12.6,0.05,text("c=1.2", 30,:red, font)),(30.97,26,text("c=3.0", 30,:red, font))])
end
savefig(f, "/Users/javier/Dropbox/Aplicaciones/Overleaf/MFPT/Figs/MFPT_Gauss.png")
exp(-γ/2)
...
begin
    ...
    for (j,δc) in enumerate(1.4:0.2:2.6)
        ...
        f = plot!(0.1:0.01:10, w-> nmean2(δc,w), s=:auto, label="c = $(δc)", frame=:box, lw=5, legend=:topleft)
        r = fpstats[fpstats[:, end] .== δc,:]
        f = plot!(sqrt.([r[i,1] for i in 1:2:size(r,1)]) .* (0.1/(2 * δc)), [r[i,3] for i in 1:2:size(r,1)],  markershapes = :auto, lw=0, markerstrokewidth=0, ms=8, label=:none)
    end
    f = vline!([exp(-γ/2)], lw=4, c=:black, ls=:dash, label=:none)
    f = plot!(size=(1500,900), leftmargin = 10Plots.mm, bottommargin = 10Plots.mm, ylabel="μ", xlabel="w", guidefontsize=25, tickfont = font(23, "Helvetica"), yscale=:log, xscale=:log)
    f = plot!(ann=[(5,15,text("c=1.2", 30,:red, font)),(2.5,50000,text("c=3.0", 30,:red, font))], legendfontsize=30)
...
begin
    ...
    for (j,δc) in enumerate(1.2:0.2:3.0)
        # f = plot!(0.1:0.01:10, w-> nmean2(δc,w) + (1 - exp(-w^2)), label=δc, frame=:box, lw=5)
        f = plot!(log(0.1):0.01:log(500), u-> log(nmean2(δc,exp(u))), frame=:box, lw=5, legend=:bottomright, s=:auto, label="c = $(δc)")
        r = fpstats[fpstats[:, end] .== δc,:]
        f = plot!(log.(sqrt.([r[i,1] for i in 1:1:size(r,1)]) .* (0.1/(2 * δc))), [log(r[i,3]) for i in 1:1:size(r,1)], markershapes = :auto, lw=0, label=:none, markerstrokewidth=0, ms=8)
    end
    f = vline!([-γ/2], lw=4, c=:black, ls=:dash, label=:none)
    f = plot!(size=(1500,1500), leftmargin = 10Plots.mm, bottommargin = 10Plots.mm, ylabel="ln(μ)", xlabel="ln(w)", guidefontsize=25, tickfont = font(23, "Helvetica"), aspect_ratio=1)
    f = plot(f, ann=[(7.5,8.3,text("c=1.2", 25,:red, font)),(7.5,15.9,text("c=3.0", 25,:red, font)), (8.3,14.1,text("} Δln(μ) = Δc²", 25,:red, font)), (5.4,4,text("1", 25,:black, font)), (6.9,6,text("1", 25,:black, font))], xlim=(-3,15))
    f = plot!(log(50):0.01:log(500),x->x+ 0.6^2 + log(√π), lw=5, c=:black, label=:none)
    f = plot!(log(50):0.01:log(500),x->0.6^2 + log(51) + log(√π), lw=5, c=:black, label=:none)
    f = plot!([log(500), log(500)],[log(50)+ 0.6^2 + log(√π), log(500)+ 0.6^2 + log(√π)], lw=5, c=:black, legendfontsize=30, label=:none)
    # f =
end
savefig(f, "/Users/javier/Dropbox/Aplicaciones/Overleaf/MFPT/Figs/MFPNR_Gauss_Log.png")
